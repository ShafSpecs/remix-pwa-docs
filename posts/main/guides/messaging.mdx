...
description: "Build an comprehensive communication system between your service worker and client."
...
self.addEventListener('message', (event) => {
  await messageHandlerForUser.handleMessage(event);
  await messageHandlerForPost.handleMessage(event);  
});
```
