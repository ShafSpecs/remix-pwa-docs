...
Before we dive into the awesomeness of push, let's clarify a few things about how imports work in the push package. A full push workflow involves both the server and client, meaning that exporting from one place would mean overlap of client and server-only apis 💥. To avoid this, `@remix-pwa/push` provides clear distinction between client and server imports. There are four main export routes in `@remix-pwa/push`:
...
- `@remix-pwa/push/server`: This is the same as above, but more verbose. Imports all server-side APIs and utilities.
...
- `@remix-pwa/push/hook`: This is the hook import route for importing [`usePush`](/docs/main/use-push) hook, a React hook for everything push in your PWA client.
