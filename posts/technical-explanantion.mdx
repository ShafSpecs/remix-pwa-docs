...
description: An technical explanantion of Remix PWA v3.0 internals
...
- Builds all your Remix routes (based on a meanifest graph), extracts all `workerLoader` and `workerAction` functions, then bundles them into the built worker file.
...
- `remix-pwa packages`: Installs all required packages for Remix PWA to work. This is a special command that is used to install all required packages for Remix PWA to work.
