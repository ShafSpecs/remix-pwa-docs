...
Remix PWA ships this feature with the `@remix-pwa/sw` package, now you can ensure an action is performed, bad connection or not.
...
function exported from `@remix-pwa/sw`.

```js {{ filename:'entry.worker.ts' }}
import { registerSync } from "@remix-pwa/sw";
...
```js {{ filename:'entry.worker.ts' }}
import { registerAllSyncs } from "@remix-pwa/sw";
...
To add the requests to the queue, you would utilise the `queueToServer` function exported from `@remix-pwa/sw`. This function takes two arguments, the tag of the sync event, and the request you want to fire.
...
import { queueToServer } from "@remix-pwa/sw";
...
I hope you didn't get surprised a seemingly working code snippet ✨. Let's look at it.
